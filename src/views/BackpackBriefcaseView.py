...
class BackpackBriefcaseView:
    def __init__(self, root, controller):
        ...
        ttk.Combobox(root, textvariable=self.modelo_var, values=["BP (Backpack)", "BF (Briefcase)"]).pack(pady=5)
    ...
    def generar_codigo(self):
        ...
        modelo = self.modelo_var.get().split()[0]  # BP o BF
        año = str(self.año_var.get())[-2:]  # Últimos 2 dígitos
        semana = str(self.semana_var.get()).zfill(2)  # Semana con 2 dígitos
        consecutivo = str(self.consecutivo_var.get()).zfill(3)  # Consecutivo con 3 dígitos
